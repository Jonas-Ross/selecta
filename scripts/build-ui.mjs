...
for (const [name, sheets] of [
  ['playlist-draft', ['pulse', 'setlist', 'timeline']],
  ['library-explorer', ['explorer']],
]) {
  let widgetHtml = await readFile(new URL(`../ui/${name}.html`, import.meta.url), 'utf8');
  ...
  await writeFile(
    new URL(`../dist/ui/${name}.html`, import.meta.url),
