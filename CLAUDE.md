...
- Directory: the branch name flattened to `<type>-<short-slug>`, under whatever root the tool uses (`.claude/worktrees/feat-play-history`, `~/.codex/worktrees/feat-play-history`). Never a hash, a generated name, or a `+`-joined path.
- A session that starts on an auto-named branch (`worktree-…`, `claude/…`, `bright-running-fox`) renames it before any work: `git branch -m <type>/<slug>`. If it is also inside an auto-named linked worktree (`agent-<hash>`), move that too: `git worktree move <old> <root>/<type>-<slug>`, then re-enter the moved path so the session's cwd follows. The primary checkout only ever gets the rename; `git worktree move` refuses a main working tree. EnterWorktree names its branch `worktree-<name>`; rename it right after creating.
