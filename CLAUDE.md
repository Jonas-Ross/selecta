...
- **`src/drafts/`** — minimal revision-checked local draft storage in `drafts.db`, separate from library refreshes. Per-occurrence entry IDs preserve intentional repeated tracks. Pins/feedback remain user intent. `src/draft_app.ts` registers the additive MCP Apps surface; `ui/` is bundled by the normal build. Only explicit draft save reaches the existing create handler.
