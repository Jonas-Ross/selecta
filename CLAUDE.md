...
`docs/audio-features.md` covers the two enrichment sources, per-source terminal status and the merge rule.
...
Shared storage and bridge types live in `src/types/`; the cross-cutting error envelope in `src/types/errors.ts`. `src/domain/` owns explicit track projections, inspection/overview transforms, and the browser-safe recent-activity window. Tool dependencies, validation/errors, freshness, and filters have dedicated modules under `src/tools/`; shared cache resource preflights live in `src/operations/resources.ts`.
