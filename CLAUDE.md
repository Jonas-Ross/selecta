...
- **`src/tools/`** — sixteen MCP handlers (`search`, `library_overview`, `get_track_context`, `inspect_tracklist`, `list_playlists`, `create_playlist`, `preview_playlist`, `add_tracks`, `remove_tracks`, `reorder_tracks`, `delete_playlist`, `set_loved`, `set_rating`, `set_note`, `refresh_library`, `enrich_features`). Thin orchestrators: validate input, query cache and/or bridge, shape response. Only layer that knows MCP exists. `search` and `library_overview` share `common.libraryFilterShape`; `inspect_tracklist` resolves and summarizes an ordered draft entirely from cached track rows.
- **`src/cache/`** — SQLite at `~/Library/Application Support/Selecta/library.db`. Tracks, playlists, playlist_tracks, audio_features, notes, refresh_log + FTS5. All model-triggered reads hit this layer. audio_features (bpm/musical_key/danceability + per-field provenance, keyed by persistent ID) sits outside the refresh cycle: a reread never wipes enrichment; rows are pruned only when their track leaves the library. notes (the model's own free text, one per track or playlist) follows the same lifecycle, and a playlist note moves with its playlist when sync reconciliation rekeys the ID.
...
v3 underway, tracked in #31–#32 and #34–#38. Play-history (#31): sparse per-refresh play/skip deltas in `play_history`, captured inside the refresh transaction — baseline on first sighting, a dropped counter resets silently (noted in `refresh_log`). Surfaces: `play_history` windows on single-seed `get_track_context`, filter-scoped `recent_activity` (30-day window) on `library_overview`, a `recent_plays` search sort, `play_deltas_recorded` on refresh (tool + CLI). Grain follows refresh cadence — deltas exist only where refreshes bracketed the listening. Approved-preview materialization (#34, PR #40): `create_playlist` can clone the verified live order of a non-empty plain user source (max 500) without resending track IDs. Preview identity across iCloud rekeys (#44): `preview_playlist` records a creation receipt when it creates the slot, and a clone of the reserved `Selecta Preview` whose ID is gone live re-resolves the slot by exact name in the bridge (one plain user match, else a loud missing/ambiguous error before creation), reporting `source.rekeyed_from` and aliasing the stale ID in the cache; arbitrary source IDs stay strictly ID-based. Ordered-draft inspection (#35): cache-only `inspect_tracklist` verification before preview/write. Co-occurrence source controls (#36): `exclude_playlist_ids` / `max_playlist_tracks` apply explicit factual filters before aggregation; `source_playlists` audits considered and excluded user playlists. Targeted enrichment (#37): optional `track_ids` on `enrich_features` attempts only those pending tracks (max 50), reports per-ID outcomes, and rejects unknown or duplicate IDs before source requests; omit it for the existing most-played backlog mode. Compact discovery (#38): `compact` on `search` / `get_track_context` keeps comparison facts while context deduplicates shared playlist names and IDs through an indexed legend. Model-persisted notes (#32): `set_note` upserts one free-text note per track or playlist (empty body clears; `create_playlist` takes an optional `note` at creation); notes ride every track and playlist surface verbatim as `note: {body, created_at, updated_at}`, survive refresh, follow playlist rekeys, and are pruned with their subject.
