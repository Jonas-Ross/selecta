...
- Work that depends on an unmerged PR is stacked, not held: `gh stack` where it runs (`.claude/skills/gh-stack` has the workflow and its limits), otherwise branch off that PR's head and open the follow-up against it. Stacked branches follow the naming above. After editing a lower layer, rebase the upper layers onto it and publish with lease protection; copying fixes between branches does not maintain stack ancestry.
