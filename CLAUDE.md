...
- **`src/tools/`** — one MCP handler per file. Thin orchestrators: validate input, query cache and/or bridge, shape response. Handlers remain transport-neutral; MCP registration lives in `server.ts` and `draft_app.ts`. `search` and `library_overview` share `common.libraryFilterShape`; `inspect_tracklist` resolves and summarizes an ordered draft entirely from cached track rows.
