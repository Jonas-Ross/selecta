...
- **`src/drafts/`** — minimal revision-checked local draft storage in `drafts.db`, separate from library refreshes. Per-occurrence entry IDs preserve intentional repeated tracks. Selection identifies the subject of explicit feedback; it never implies an action. Draft cards use Setlist without pins, with a collapsible duration timeline, artist strip and optional tempo/key lanes. `src/draft_app.ts` registers the additive MCP Apps surface; `ui/` is bundled by the normal build, which runs `npm run typecheck:ui`. `ui/playlist-draft.js` wires the host into the injected `ui/draft-controller.ts`; `ui/draft-state.ts` owns pure response/state handling, and `src/drafts/contracts.ts` shares browser-safe schemas with storage. `ui/render.js` and `ui/timeline.js` paint the card from state, updating rows and blocks in place keyed by entry ID (`ui/reconcile.js`), and are unit-tested against the controlled DOM in `test/dom.ts`. Only explicit draft save reaches the shared creation operation; direct creation and draft saving retain observed outcomes across local persistence failures.
