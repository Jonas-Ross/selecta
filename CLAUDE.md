...
- A session that starts in an auto-named worktree or branch (`agent-<hash>`, `worktree-…`, `claude/…`, `bright-running-fox`) fixes both before any work: `git branch -m <type>/<slug>`, `git worktree move <old> <root>/<type>-<slug>`, then re-enter the moved path so the session's cwd follows. EnterWorktree names its branch `worktree-<name>`; rename it right after creating.
