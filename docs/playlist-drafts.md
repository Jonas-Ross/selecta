...
**Load into Selecta Preview** calls `preview_playlist_draft({draft_id, revision})`, explicitly replacing the shared audition slot and linking this draft. It never starts playback. Subsequent requested ordered-track changes call the bridge once as part of `edit_playlist_draft`, without redundant confirmation. Name, selection, feedback and identical ordered IDs do not write. Permanent Save remains explicit.
...
**Open preview in Music.app** calls `open_preview` with the complete draft order, including repeated IDs. It reveals exactly one reserved plain-user preview whose live sequence matches, then brings Music forward. It does not replace the preview or start playback. Load the draft into Selecta Preview before opening; reconcile missing, ambiguous or stale targets explicitly. Saving remains separate. See [Music.app field notes](music-app.md#opening-the-preview-from-a-draft) for the live Open smoke and remaining #89 playback scope.
