...
The user confirmed the isolated card looked correct, then reported a persistent outer scrollbar. The document shell now has zero margin/padding, and sizing observes the card host itself instead of measuring the iframe document. Reports round up with two CSS pixels for host border/rounding differences. A controlled regression covers initial size, unchanged reports, and content growth/shrinkage; narrow preview inspection showed no outer scrollbar. Actual-host sizing recheck remains pending.
