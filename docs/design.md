...
- **iCloud Sync Library sometimes duplicates a freshly created playlist** — a second copy with identical tracks and a different persistent ID appears as sync settles. Non-deterministic and Apple-side: observed ~10s after one scripted create, absent for the identical call minutes later, and the same library had pre-existing doubles of Apple's own playlists (including a smart playlist Selecta can't create). Deliberately not worked around in code — a settle-delay is a timing heuristic, and an auto-dedupe sweep risks sync resurrecting the deletion (see transient-ID note above); both violate the no-hidden-fallbacks identity. The `create_playlist` and `preview_playlist` descriptions carry the failure-mode hint instead (a twinned preview slot is benign: `replacePlaylist` keeps overwriting the first name match), and the README troubleshooting section covers it for users.
