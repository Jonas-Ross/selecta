...
untouched, and the note left behind on the stale ID is collected by that same
prune. Reconciliation reports the collision instead of resolving it, because the
two notes need not describe the same playlist.
