...
Run `npm run preview:explorer`, then open `http://127.0.0.1:8767` (override with `SELECTA_PREVIEW_PORT`). The existing development fixture host serves the production bundle, an in-memory synthetic library and simulated refresh receipts. It never opens the user cache or calls Music.app. Width controls offer 760px, 553px and 390px; surface controls exercise light/dark themes and representative Claude/Codex styles, including the 720px cap. Fixtures cover normal slices, missing metadata, 1,000 tracks with 65 raw genres, and an empty library. **Latest interaction** shows exact context or the message instead of sending it to an agent. Source edits reload the card and reset temporary selection.
