...
Direct creation and draft saving share a creation operation. It holds the Music operation lock across preflight, the single bridge attempt and local persistence, but never holds a SQLite transaction across the Music.app call. Once the bridge returns an observed destination, its cached membership, creation receipt, optional note and applicable preview-source rekey commit together. A persistence failure rolls back that local transaction and returns `cache_unavailable` with `partial_write.playlist_id` and the exact `observed_track_ids`, including duplicates or an empty readback. Inspect the returned target and refresh the cache; never recreate it to repair persistence. A rolled-back creation receipt or note cannot be reconstructed by refresh alone; retain the returned identity and any intended note when choosing explicit recovery.

Only validated pre-creation script guards prove that no write began. Subprocess errors, including permission/app-not-running signatures, do not establish a safe retry. Creation/clone guards cannot discard conflicting destination or partial-write evidence. If the full response is malformed, a separately validated target ID and any valid ordered IDs survive in a `jxa_error.partial_write` diagnostic receipt; invalid or absent observed IDs are omitted, not reported as an empty playlist. That malformed result is never persisted as successful cache state. If creation and its cache transaction committed but lock cleanup fails, `operation_cleanup_failed` returns `creation_committed: true`, the normal creation fields and saved note, the exact observed receipt, and `lock_path`. Recover the stale lock under the interrupted-call procedure above; this committed result needs no refresh or repeat creation. If persistence and cleanup both fail, the `cache_unavailable` response retains both causes, the diagnostic target/order, and the lock path.
...
Non-destructive live smoke (2026-09-13): the production bridge resolved and revealed the existing 18-entry plain user preview; a follow-up read preserved the full order. No playlist contents changed and no playback command ran. This confirms the Open bridge call, not playback behavior or rendering in an MCP host.
