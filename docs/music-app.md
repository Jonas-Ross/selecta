...
Only validated pre-creation script guards prove that no write began. Subprocess errors, including permission/app-not-running signatures, do not establish a safe retry. Creation/clone guards cannot discard conflicting destination or partial-write evidence. If the full response is malformed, a separately validated target ID and any valid ordered IDs survive in a `jxa_error.partial_write` diagnostic receipt; invalid or absent observed IDs are omitted, not reported as an empty playlist. That malformed result is never persisted as successful cache state. Lock cleanup failure preserves the settled creation outcome too; the cache may already have committed when cleanup fails. Remove a stale lock only under the interrupted-call procedure above.
