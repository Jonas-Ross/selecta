...
The old `npm run verify:echo` create/poll/delete probe is retired because its single-survivor assertion depended on automatic duplicate removal. The command now exits unsuccessfully with guidance and performs no Music.app operations. Use `npm test` for fixture coverage. To inspect a live sync event, explicitly run `refresh` and inspect `sync_reconciliation.ambiguous`; deciding which copy to delete remains a separate user action.
