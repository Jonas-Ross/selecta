...
Refresh and Music.app writes hold a per-database `library.db.music.lock` directory across preflight, scripting, and cache updates. Enrichment independently holds `library.db.enrich.lock` across its whole run, including server-requested cooldowns. A competing CLI/MCP call fails with `operation_busy`; cached reads remain available. These locks coordinate Selecta processes using the same database, not edits made by Music.app or other applications.
