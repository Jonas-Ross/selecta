...
export const REFRESH_LIBRARY_DESCRIPTION = `Reread the entire Music.app library into the local cache. Takes seconds to a minute depending on library size, and requires Music.app automation permission. Only call when the user asks for a refresh, when cache_age_hours is null (never populated), or when stale-cache errors (track_not_found) suggest the library changed. Also worth one call a few minutes after create_playlist: it reconciles unambiguous rekeys and reports identical copies under sync_reconciliation.ambiguous; refresh never deletes playlists. Ask the user which ambiguous copy to keep before calling delete_playlist and reports what it did in sync_reconciliation. sync_reconciliation.note_conflicts names rekeys whose destination already had its own note: that note stands and the rekeyed playlist's note did NOT travel, so re-read the note via list_playlists before assuming what it says. audio_features_pending in the response counts tracks enrich_features hasn't attempted yet (a refresh never wipes existing features). Each refresh also records per-track play/skip deltas since the previous one (play_deltas_recorded) — the play-history record behind recent_activity and the recent_plays sort; more regular refreshes give it finer grain. Never call it routinely before searches.`;
