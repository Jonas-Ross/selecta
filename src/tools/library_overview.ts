...
export const LIBRARY_OVERVIEW_DESCRIPTION = `Returns the shape of the user's owned library (or a filtered slice) so you can orient before building: genre distribution, decade histogram, top artists by track count, total runtime, and signal coverage (loved / rated / never-played / a rating histogram). Use it for vibe-only requests ("something jazzy and late-night") — get the map first, then call search for the specific slices. Accepts the same optional filters as search (all ANDed); with no filters it describes the whole library. Counts are RAW: genres are NOT normalized, so "Hip-Hop", "Hip-Hop/Rap", and "Rap" are separate buckets — merge them yourself if you mean one thing. top_artists is "most tracks owned", a fact about the collection, not a recommendation. genres caps at the top 50 (the rest summarized in genres_other); top_artists caps at 25 (artists_total gives the full distinct count). If cache_age_hours is null the cache has never been populated: call refresh_library once.`;
