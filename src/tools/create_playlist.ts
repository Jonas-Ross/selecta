...
export const createPlaylistInputShape = {
  ...
  source_playlist_id: z
    ...
    .optional()
    .describe(
      'Existing playlist to clone from its current live Music.app order. Mutually exclusive with track_ids.',
    ),
...
export async function handleCreatePlaylist(
  ...
  try {
    ...
    const resolvedSourceId = cache.resolvePlaylistId(source_playlist_id!);
    if (cache.getPlaylist(resolvedSourceId) === null) {
      return {
        error: 'playlist_not_found',
        hint: `No source playlist ${source_playlist_id} in the cache. Use IDs exactly as returned by list_playlists; if the library changed, run refresh_library.`,
      };
    }

    const result = await deps.bridge.clonePlaylist({
      name,
      sourcePlaylistId: resolvedSourceId,
      description,
    });
